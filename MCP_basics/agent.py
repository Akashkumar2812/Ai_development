...
async def main():
    ...
    print()
    tool_args = {}
    if tool_name == "generate_qr":
       text = input("Enter text or URL for QR code: ")
    tool_args = {"text": text}
    #step4 Ex3ecute the seelected tool on the server and return the result
    tool_result = await execute_tool(client, tool_name,tool_args)
